...
async function startBot() {
    ...
    const sock = makeWASocket({
        ...
        browser: ["WHITEDEVIL TM", "Chrome", "3.0"] 
    })

    // --- Pairing Code Logic ---
    if (!sock.authState.creds.registered) {
        console.log("🚀 WHITEDEVIL TM: Connecting to WhatsApp...");
        setTimeout(async () => {
            try {
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (connection === 'close') {
            ...
            console.log('✅ WHITEDEVIL TM IS ONLINE!');
        }
    })
...
*© 2026 WHITEDEVIL PRODUCTION*`;
        ...
        try {
            ...
            if (lowText.startsWith('.img')) {
                ...
                await sock.sendMessage(chatID, { image: { url: imgUrl }, caption: `🎨 *Masterpiece by WHITEDEVIL TM*` }, { quoted: msg });
            }

            // ALIVE
            if (lowText === '.alive') {
                const aliveBody = `*WHITEDEVIL TM: STATUS REPORT* 🛡️\n\n*🚀 Uptime:* ${(process.uptime() / 60).toFixed(1)}m\n*📟 RAM:* ${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB\n*👤 Lead:* Nisala Sandakan\n\n*Status: Online & Optimized*`;
                await sock.sendMessage(chatID, { 
                    text: aliveBody,
                    contextInfo: {
                        externalAdReply: {
                            title: "WHITEDEVIL TM: ALL SYSTEMS GO",
